...
def fromFullOptions(options_og):
    ...
    options['MSMScalibration'] = options_og._data.get('MSMScalibration',
                                                      None)  # this is SHit because there is no get in this custom dict!!!
...
def getMinMaxMFQLMass(root_mfql_dir, forMS2=False):
    ...
    defs = set() #TODO should be on ordered set to avoid duplicates but convention is first is the precursor
    ...
    def getDefMinMax(definition):
        ...
        if not isinstance(elemSeq, SCConstraint):
            return elemSeq.getWeight(),elemSeq.getWeight()# its a elemnet sequence ...ie no range
        ...
        for item in reader:
            ...
            if msX == 1 and ms1Delta is None:
                ...
                if ms2Masses is None: #initialize
                    ms2Masses = m
                    continue
                if m.min() > ms2Masses.max() or m.max() < ms2Masses.min(): # it complements
                    ms2Masses = np.concatenate((ms2Masses,m))
                elif m.max()-m.min() > ms2Masses.max() - ms2Masses.min(): # its a greater range
                    ms2Masses = m
                if ms2Masses.max()-ms2Masses.min() > ms1Delta.dist: # we have a range similar to the ms1
                    ms2Delta = getDelta(ms2Masses)
            if ms1Delta and ms2Delta: break
        if ms2Delta is None:
            ms2Delta = getDelta(ms2Masses)
...
def setOptions_fromImputPaths(input_files_dir, root_mfql_dir):
    ...
    options['MSMSmassrange'] = 0,getMinMaxMFQLMass(root_mfql_dir)[1] # TODO handle neutral loss neutral charege takes a fragement
    ...
    options['MSthreshold'] = 0.00001 #intensity threshold shooul be addressed by jaco filter
    options['MSMSthreshold'] = 0.00001
    options['precursorMassShift'] = 0.0
    options['precursorMassShiftOrbi'] = None
    options['MScalibration'] = ''#options_og['MScalibration'] #todo find most intense peak and use that?... should actually use a standard
    options['MSMScalibration'] = None
    options['selectionWindow'] = 0.5 #TODO why does this affect the results so much
    options['MSMSresolution'] = getTolerance(11.11) #todo why this malkes different results
    options['MSMSresolutionDelta'] = resolutionDeltas.ms2
    options['isotopicCorrectionMS'] = True
    options['isotopicCorrectionMSMS'] = True
    options['complementMasterScan'] = False
    options['statistics'] = False
    options['optionalMStolerance'] = getTolerance(2.0) #todo why this gets differrent results
    options['optionalMSMStolerance'] = getTolerance(2.0)
    options['noPermutations'] = False # we keep permutations, why remove them? maybe only not report them
    options['intensityCorrection'] = False
    options['noHead'] = False
