...
@pytest.fixture
def get_options():
    options = read_options(r"test_resources\ms2_strained\project_data\PRM_import.lxp")
    return options
...
@pytest.fixture
def getMfqlFiles():
    p = Path(r"test_resources\ms2_strained\mfqls")
    mfqls = p.glob("*.mfql")
...
def test_masterscan_manual(get_options, get_masterscan):
    ...
    with open(r"test_resources\ms2_strained\reference\masterscan_1.pkl", "rb") as f:
...
def test_masterescan_automatic(get_no_res_options, get_no_res_masterscan):
    ...
    with open(r"test_resources\ms2_strained\reference\masterscan_2.pkl", "rb") as f:
        reference = pickle.load(f)
    assert compareMasterScans(masterscan, reference)
...
def test_mfql_automatic(get_no_res_masterscan, get_no_res_options, getMfqlFiles):
    with open(r"test_resources\ms2_strained\reference\result_2.pkl", "rb") as f:
        reference = pickle.load(f)
