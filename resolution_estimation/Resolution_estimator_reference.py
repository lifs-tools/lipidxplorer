...
df.columns =['scanNum', 'filterLine', 'retTime', 'm', 'i', 'r',
       'x', 'y', 'z']
...
small_df = df.loc[(df.rel_i > 0.01) & (df.calc_r > 10000)].plot.scatter(x='mean_r', y='calc_r')
...
df.loc[ (df.rel_i > 0.01) & (df.rel_calc_r > 0.95)].plot.scatter(x='mean_r', y='max_calc_r')
...
sel_df = df.loc[ (df.rel_i > 0.01) & (df.i < 5.125122e+04) & (df.rel_calc_r > 0.95)] # df.i is because q[50]
# sel_df['mean_max_calc_r'] = df.groupby('round_m')['max_calc_r'].transform('mean')
# sel_df.plot.scatter(x='mean_r', y='mean_max_calc_r' )
plt.scatter(x=sel_df['mean_r'], y=sel_df['max_calc_r'], c=sel_df['m']/10, s=sel_df['i']/100)
...
sel_df.loc[(sel_df.mean_r <72000) & (sel_df.max_calc_r <31000)].sort_values('i', ascending= False)
...
popt_calc, pcov_calc = curve_fit(func, sel_df.m,sel_df.max_calc_r, bounds=(0, [1., 1., 1.]))
popt, pcov = curve_fit(func, sel_df.m, sel_df.mean_r,bounds=(0, [1., 1., 1.]))
...
plt.plot(sel_df.m, func(sel_df.m, *popt), 'g--',label='fit: a=%5.3f, b=%5.3f, c=%5.3f' % tuple(popt))
plt.legend()
plt.show()
...
popt_calc, pcov_calc = curve_fit(func, x, y_calc , bounds=(0, [1e6, 1., 1e3]))
print(popt)
print(popt_calc)
...
plt.plot(x, func(x, *popt_calc), 'b',label='fit_calc: a=%5.3f, b=%5.3f, c=%5.3f' % tuple(popt_calc))
plt.legend()
plt.show()
