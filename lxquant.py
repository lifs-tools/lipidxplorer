...
def lxquant(input_file, istd_file):
    ...
    df1_istd_ratio = df1_istd_tmp.apply(lambda x: x[-1]/x[1:], axis=1, result_type='expand')# started at 1 to drop mass
    ...
    for i in range(len(df1_istd_ratio)):
        ...
        df1_sub = df1[df1['CLASS'] == an_istd['LipidClass']] # the identifiactions for that standard
        #select columns
        del an_istd['LipidClass'] # this cant be multiplied
        cols = set(df1_sub.columns).intersection(set(an_istd.index)) # the colmuns in common
