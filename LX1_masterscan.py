...
def make_lx1_masterscan(options) -> MasterScan:
    ...
    ms1_agg_peaks = ms1_scans_agg(ms1_agg_peaks)
...
def ms1_scans_agg(ms1_agg_peaks):
...
def MS2_dict_generator(ms2_agg_peaks, samples, polarity):
    for precursor_mz, gdf in ms2_agg_peaks.groupby('precursor_mz'):
        MSMSEntry_list = [ms2entry_factory(*args) for args in MSMSEntry_list_generator(gdf, samples, polarity)]
