...
class _WorkerLog:
    ...
    every worker is appending to is cheap on APFS (~70us) but expensive on
    Windows, where an on-access virus scanner runs on each open; a
    14-sample run took 441s instead of 227s.
...
def _install_worker_logging(log_file, context="", verbose=False):
    ...
    worker's own progress, and capturing it cost a 14-sample Windows run
    441s against 227s, so it is off unless asked for.
