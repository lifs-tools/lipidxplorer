...
class Project(Options):
	...
	def writeOptionsToIni(self):
		...
		self.confParse.set(configuration, 'timerange', "(%f,%f)" % (o['timerange'][0], o['timerange'][1]))
		self.confParse.set(configuration, 'selectionWindow', o['selectionWindow'])
		str = ''
		...
		self.confParse.set(configuration, 'MSmassrange', "(%f,%f)" % (o['MSmassrange'][0], o['MSmassrange'][1]))
		self.confParse.set(configuration, 'MSMSmassrange', "(%f,%f)" % (o['MSMSmassrange'][0], o['MSMSmassrange'][1]))
		self.confParse.set(configuration, 'MSresolution', o['MSresolution'].tolerance)
		self.confParse.set(configuration, 'MSMSresolution', o['MSMSresolution'].tolerance)
		...
		self.confParse.set(configuration, 'MSresolutionDelta', str(o['MSresolutionDelta']))
		self.confParse.set(configuration, 'MSMSresolutionDelta', str(o['MSMSresolutionDelta']))
		...
		self.confParse.set(configuration, 'precursorMassShiftOrbi', o['precursorMassShiftOrbi'])
		self.confParse.set(configuration, 'alignmentMethodMS', o['alignmentMethodMS'])
		self.confParse.set(configuration, 'alignmentMethodMSMS', o['alignmentMethodMSMS'])
		self.confParse.set(configuration, 'scanAveragingMethod', o['scanAveragingMethod'])
...
class GUIProject(Project):
	...
	def formatOptions(self):
		...
		if not o['timerange'] is None:
			self.options_formatted['timerange'] = (o['timerange'].split(',')[0].strip('() '),
		...
		if not o['MSmassrange'] is None:
			self.options_formatted['MSmassrange'] = ((o['MSmassrange'].split(',')[0].strip('() ')),
					(o['MSmassrange'].split(',')[1].strip('() ')))
		if not o['MSMSmassrange'] is None:
			self.options_formatted['MSMSmassrange'] = ((o['MSMSmassrange'].split(',')[0].strip('() ')),
