...
def add_Sample(
	...
	# get MS1 scans
	get_ms1Scans = [(t, mz, sn, sm, pol, tic, np, bp) for
			t, mz, sn, st, sm, pol, tic, np, bp
	...
	if not ps:
		...
		ps.addScansToAvg(str(specFile.split(os.sep)[-1]),
				str(specFile), nb_ms_scans, nb_ms_peaks)
	...
	for t, precursor, sn, sm, pol, tic, np, bp in get_ms2Scans:
		...
		ps.addMSMSScan(str(specFile.split(os.sep)[-1]),
				str(specFile), precursor, nb_ms_peaks)
