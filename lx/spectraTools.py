...
def recalibrateMS(sc, listRecalibration, isCalctol = False):
	...
	if listRecalibration and len(listRecalibration) > 0:
		...
		for key in sc.listSamples:
			...
			if isCalctol:
				sc.options['MStolerance'].tolerance = calc_tol(sc.dictSamples[key].listPrecurmass) / 2
...
def recalibrateMSMS(sc,  listRecalibrationMSMS, isCalctol, listRecalibrationMS = None, ms1_recal_table = None):
	...
	if listRecalibrationMSMS and len(listRecalibrationMSMS) > 0:
		for key in sc.listSamples:
			...
			if isCalctol:
				...
				sc.options['MSMStolerance'].tolerance = sc.options['MSMStolerance'].tolerance / 2
