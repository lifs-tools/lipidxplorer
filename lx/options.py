...
class optionsDict(DictMixin):
    ...
    def isEmpty(self, option):
        return (self._data[option] is None) or \
                (self._data[option] == '') or \
                (self._data[option] == [])
...
class Options:
    ...
    def __init__(self, options = None):
        ...
        self.options['scanAveragingMethod'] = None
        self.options['isotopicCorrection_MSMS'] = None # here starts the Set debugging Options from the Debug menu
        ...
        self.options['intensityCorrection'] = None # here starts the output options menu
        ...
        self.options['mzXML'] = None # option key used in lpdxImport.py, substituted by 'dataType'
    ...
    def isEmpty(self, option):
        ...
        return (option is None) or (option == '') or\
                (option == '()') or (option == '(,)')
            ...
            if m is None:
                if not self.options['optionalMStoleranceType'] is None and not self.options['optionalMStoleranceType'] == '':
                    ...
                    if not m is None:
                        ...
                        raise LipidXException("The entry for the MS tolerance is corrupt.")
                else:
                    raise LipidXException("The entry for the MS tolerance is corrupt or the type is missing.")
            else:
                if float(m.group(1)) < 0.0:
                    raise LipidXException("The tolerance value for MS should be >= 0")
            ...
            if m is None:
                if not self.options['optionalMSMStoleranceType'] is None and not self.options['optionalMSMStoleranceType'] == '':
                    ...
                    if not m is None:
                        ...
                        raise LipidXException("The entry for the MS/MS tolerance is corrupt.")
                else:
                    raise LipidXException("The entry for the MS/MS tolerance is corrupt or the type is missing.")
            else:
                if float(m.group(1)) < 0.0:
                    raise LipidXException("The tolerance value for MS/MS should be >= 0")
    ...
    def testOptions(self):
        ...
        if not self.isEmpty('timerange'):
            timerange = (float(self.options['timerange'].split(',')[0].strip('() ')),
            ...
            if not timerange[0] <= timerange[1]:
                raise LipidXException("Second value of the time range is smaller then the first, which makes no sense ...")


        if not self.isEmpty('MScalibration'):
            for mass in self.options['MScalibration'].split(','):
                if float(mass) < 0:
                    raise LipidXException("The m/z value %s in the MS calibriation list is negative." % mass)
        if not self.isEmpty('MSMScalibration'):
            for mass in self.options['MSMScalibration'].split(','):
                if float(mass) < 0:
                    raise LipidXException("The m/z value %s in the MS/MS calibriation list is negative." % mass)


        if not self.isEmpty('MSmassrange'):
            MSmassrange = (float(self.options['MSmassrange'].split(',')[0].strip('() ')),
            ...
            if not MSmassrange[0] <= MSmassrange[1]:
                raise LipidXException("Second value of the m/z range for MS is smaller then the first, which makes no sense ...")

        if not self.isEmpty('MSMSmassrange'):
            MSMSmassrange = (float(self.options['MSMSmassrange'].split(',')[0].strip('() ')),
            ...
            if not MSmassrange[0] <= MSmassrange[1]:
                raise LipidXException("Second value of the m/z range for MS/MS is smaller then first, which makes no sense ...")
        ...
        if not self.isEmpty('MStolerance'):
            ...
            if m is None:
                if not self.options['MStoleranceType'] is None and not self.options['MStoleranceType'] == '':
                    ...
                    raise LipidXException("The entry for the MS tolerance is corrupt or 'MStoleranceType' is missing.")
            else:
                if float(m.group(1)) < 0.0:
                    raise LipidXException("The tolerance value for MS should be >= 0")
        ...
        if not self.isEmpty('MSMStolerance'):
            ...
            if m is None:
                if not self.options['MSMStoleranceType'] is None and not self.options['MSMStoleranceType'] == '':
                    ...
                    raise LipidXException("The entry for the MS/MS tolerance is corrupt or 'MStoleranceType' is missing.")
            else:
                if float(m.group(1)) < 0.0:
                    raise LipidXException("The tolerance value for MS/MS should be >= 0")
    ...
    def formatOptions(self):
        ...
        if not self.isEmpty('timerange'):
            self.options_formatted['timerange'] = (float(o['timerange'].split(',')[0].strip('() ')),
                    float(o['timerange'].split(',')[1].strip('() ')))


        if not self.isEmpty('MScalibration') and len(o['MScalibration']) > 0:
            self.options_formatted['MScalibration'] = [float(x) for x in o['MScalibration'].split(',')]
        if not self.isEmpty('MSMScalibration') and len(o['MSMScalibration']) > 0:
            self.options_formatted['MSMScalibration'] = [float(x) for x in o['MSMScalibration'].split(',')]


        if not self.isEmpty('MSresolutionDelta'):
            self.options_formatted['MSresolutionDelta'] = float(o['MSresolutionDelta'])
        if not self.isEmpty('MSMSresolutionDelta'):
            self.options_formatted['MSMSresolutionDelta'] = float(o['MSMSresolutionDelta'])
        ...
        if not self.isEmpty('MSmassrange'):
            self.options_formatted['MSmassrange'] = (float(o['MSmassrange'].split(',')[0].strip('() ')),
                    float(o['MSmassrange'].split(',')[1].strip('() ')))
        if not self.isEmpty('MSMSmassrange'):
            self.options_formatted['MSMSmassrange'] = (float(o['MSMSmassrange'].split(',')[0].strip('() ')),
                    float(o['MSMSmassrange'].split(',')[1].strip('() ')))

        if not self.isEmpty('MSresolution'):
                self.options_formatted['MSresolution'] = TypeTolerance('res', float(o['MSresolution']))
        if not self.isEmpty('MSMSresolution'):
                ...
                if not o['MStoleranceType'] is None and not o['MStoleranceType'] == '':
                    ...
                    if not m is None:
                        try:
                            ...
                            raise LipidXException("No tolerance value given. Please input a value greater than zero.")
            else:
        ...
        if not self.isEmpty('MSMStolerance'):
            ...
            if m is None:
                ...
                self.options_formatted['MSMStolerance'] = TypeTolerance(m.group(3), float(o['MSMStolerance']))
        ...
        if not self.isEmpty('optionalMSMStolerance'):
            ...
            if m is None:
                ...
                self.options_formatted['optionalMSMStolerance'] = TypeTolerance(m.group(3), float(o['optionalMSMStolerance']))

        if not self.isEmpty('optionalMStoleranceType'):
            self.options_formatted['optionalMStoleranceType'] = o['optionalMStoleranceType']
    ...
    def formatOptionsRun(self):
        ...
        if not o['optionalMSthresholdType'] is None:
            self.options_formatted['optionalMSthresholdType'] = o['optionalMSthresholdType']
        if not o['optionalMSMSthresholdType'] is None:
            self.options_formatted['optionalMSMSthresholdType'] = o['optionalMSMSthresholdType']
