...
class ChooseGroupsFrame(wx.Dialog):
	...
	def __init__(self, *args, **kwds1):
		...
		self.font_labels = wx.Font(10, wx.SWISS, wx.NORMAL, wx.NORMAL, False)#, 0, 0, wx.FONTENCODING_SYSTEM))
		self.font_ctrl = wx.Font(9, wx.SWISS, wx.NORMAL, wx.NORMAL, False)#, 0, 0, wx.FONTENCODING_SYSTEM))
		...
		self.label_left = wx.StaticText(self, -1, "Select one or more samples and press \n'Add to group' to form a new group.")
		...
		self.dragList_right = DragTree(self, style=wx.TR_HAS_BUTTONS|wx.TR_HIDE_ROOT|wx.TR_MULTIPLE|wx.TR_DEFAULT_STYLE, size = (300,200))
		...
		self.sizer_rightPart.Add(self.button_removeGroup, 0, wx.ALIGN_CENTER|wx.ALL, 5)
		...
		for item in self.items:
			...
			self.dragList_left.InsertStringItem(maxint, item, self.dragList_left.fileidx)
		...
		for item in selectedGroups:
			...
			if m.match(self.dragList_right.GetItemText(item)):
				...
				if first.IsOk(): # are there childrens?
					...
					for i in treeItems:
						#self.dragList_left.InsertStringItem(self.dragList_left.GetItemCount(), i[1])
						self.dragList_left.InsertImageStringItem(self.dragList_left.GetItemCount(), i[1], self.dragList_left.fileidx)
						self.dragList_right.Delete(i[0])

				self.dragList_right.Delete(item)

			else:
				self.dragList_left.InsertImageStringItem(self.dragList_left.GetItemCount(), self.dragList_right.GetItemText(item), self.dragList_left.fileidx)
				self.dragList_right.Delete(item)
