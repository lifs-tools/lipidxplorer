...
def p_getQueryName(p):
    ...
    if p[1] in listQueryNames:
        raise SyntaxErrorException(
            "A double query name ('%s') occured. This must not be!" % p[1],
            mfqlObj.filename,
            mfqlObj.queryName,
            p.lexer.lineno,
        )
...
def p_error(p):
    if not p:
        ...
        raise SyntaxErrorException(
            "SYNTAX ERROR at EOF", mfqlObj.filename, mfqlObj.queryName, -1
        )
    else:
        print(
            "Syntax error after '%s'\nfile: \t%s\nquery: \t%s\nline: \t%s"
            % (p.value, mfqlObj.filename, mfqlObj.queryName, p.lexer.lineno)
        )
        raise SyntaxErrorException(
            None, mfqlObj.filename, mfqlObj.queryName, p.lexer.lineno
        )
