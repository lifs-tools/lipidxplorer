...
def isotope(
    ...
    WINDOW_SIZE = 2 ** int(N.ceil(N.log(float(WINDOW_SIZE)) / N.log(2.0)))
    ...
    if isinstance(elemH, type([])):
        elemH = elemH[0]
    if isinstance(elemC, type([])):
        elemC = elemC[0]
    if isinstance(elemO, type([])):
        elemO = elemO[0]
    if isinstance(elemN, type([])):
        elemN = elemN[0]
    if isinstance(elemP, type([])):
        elemP = elemP[0]
    if isinstance(elemS, type([])):
        elemS = elemS[0]
    M = N.array(
    ...
    A[6][0, :] = [
        100000,
        1.00000,
    ]  #                   % for shifting mass so that Mmi is
