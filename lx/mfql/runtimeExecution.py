...
class TypeResult:
	...
	def generateResultSC(self):
		...
		resultSC.sort(key=lambda x: x.precurmass)
 ...
 ##################
	...
	def generateReport(self, options = {}):
		...
		#print("result------------------------------------------------------generateReport-------:",self)
		self.listHead = []
		...
		for query in list(self.dictQuery.values()):
			...
			for vars in query.listVariables:
				...
				for report in query.listReport:
					#print("self.mfqlObj.queryName--------------------in generateReport------report:",self.mfqlObj.queryName, report)
					...
					if len(report) == 3:
						...
						if isinstance(report[2], type([])):
							...
							for attr in listAttributes[1:]:
								strCorr += ",%.9f" % attr
							strCorr += ')'
							# strCorr = "(" + ", ".join("'%s'" % x for x in listAttributes) + ")"
							# for attr in listAttributes[1:]:
							# 	strCorr += ",\'%s\'" % attr
							# strCorr += ')'
       ...
       ########################### ballal edited  ##################################
							...
							try:
								# Fix variable references in brackets: [X] -> ['X']
								strCorr = noString.sub(r"['\g<1>']", strReportVars.strip('"'))

								try:
									# Ensure strCorr is a proper Python tuple
									value_tuple = eval(strCorr, {}, vars)

									# Remove wrapping quotes from format string if present
									fmt_str = report[1]
									if (fmt_str.startswith('"') and fmt_str.endswith('"')) or (fmt_str.startswith("'") and fmt_str.endswith("'")):
										fmt_str = fmt_str[1:-1]

									# Format the string safely
									str_result = fmt_str % value_tuple

									rpt[report[0]] = str_result

								except Exception as e:
									print(f"[DEBUG] Eval failed for {report[0]}: {report[1]} % {strCorr}")
...
#############################################################
				...
				#print("self.listHead----------------------#######",self.listHead)
	...
	def removePermutations(self):
		...
		for query in self.dictQuery:
			...
			if self.dictQuery[query].listVariables:
				...
				# MS1 only
				if all(v.scope == "MS1+" for v in list(self.dictQuery[query].listVariables[0].values())):
					for i in self.dictQuery[query].listVariables:
						listVar.append(sorted(list(i.items()), key=lambda x: x[1].mass))

				# with MS2
				else:
					for i in sorted(self.dictQuery[query].listVariables, key=lambda d: abs(next(v for v in list(d.values()) if v.scope in ["MS1+", "MS1-"]).errppm)):
						listVar.append(sorted(list(i.items()), key=lambda x: x[1].mass))
				# /FIX
