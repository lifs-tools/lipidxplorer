...
_lexer = re.compile(r"[A-Z][a-wyz]*|\d+|[\{\}]|<EOS>|\s|\[\d{1,6}\.\.\d{1,6}\]|\[\d{1,6}(,\d{1,6})*\]|;|db\([-+]?\d{1,3}(\.\d{1,4})?,[+-]?\d{1,3}(\.\d{1,4})?\)|chg\([+-]?\d\)").match
...
def parse_sequence():
	...
	while ttype in (NAME, SP, RANGE, ENUM, DB, CHARGE):
		...
		if ttype == RANGE:
			...
			if not thisguy:
				raise SyntaxErrorException("No space is allowed between an element and its count or range", None, None, None)
