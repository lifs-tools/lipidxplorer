...
def lpdxImportCLI(projpath=None):
    ...
    optParser.add_option(
        "--MSfilter",
        dest="MSfilter",
        help="overwrites/specifies 'MSfilter' of the *.ini file, minimum filter threshold for repeated ions in MS1, between 0 and 1",
        metavar="ARG",
    )
    ...
    optParser.add_option(
        "--MSMSfilter",
        dest="MSMSfilter",
        help="overwrites/specifies 'MSMSfilter' of the *.ini file, minimum filter threshold for repeated ions in MS2, between 0 and 1",
        metavar="ARG",
    )
    ...
    if cliOptions.prj is not None:
        ...
        project.options["dumpMasterScanFile"] = (
            os.path.splitext(args[0])[0] + "-dump.csv"
        )
