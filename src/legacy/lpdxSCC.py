    ...
    if len(args) > 0:
        ...
        if not args[0] or not args[1]:
            ...
            if args[0] == "msc":
                ...
                if rsltlist == []:
                    print(
                        "No sum composition found for %s with m/z %2.f"
                        % (elscp, float(args[1]))
                    )
                for mass, scp in rsltlist:
                    print(
                        "%.4f" % mass,
                        scp,
                        "error: %.4f" % (float(args[1]) - scp.getWeight()),
                    )
                ...
                while peak < max(len(spec1), len(spec2)) - 1:
                    ...
                    if (
                        spec2[peak][0] - t < spec1[peak][0]
                        and spec1[peak][0] < spec2[peak][0] + t
                    ):
                        if peak < len(spec1) - 1 and peak < len(spec2) - 1:
                            if not (
                                spec2[peak][0] - t < spec1[peak + 1][0]
                                and spec1[peak + 1][0] < spec2[peak][0] + t
                            ):
                                vSpec1.append(spec1[peak])
                                vSpec2.append(spec2[peak])

                            else:
                                if abs(spec1[peak][0] - spec2[peak][0]) > abs(
                                    spec1[peak + 1][0] - spec2[peak][0]
                                ):
                                    vSpec1.append(spec1[peak])
                ...
                print(
                    "dot product: %.4f, similarity: %.2f %%"
                    % (sum, 100 - (phi * 100) / math.pi)
                )
                ...
                sVSpec1 = math.sqrt(
                    len(vSpec1) * sumVSpec1quad - (sumVSpec1 * sumVSpec1)
                )
                sVSpec2 = math.sqrt(
                    len(vSpec2) * sumVSpec2quad - (sumVSpec2 * sumVSpec2)
                )
