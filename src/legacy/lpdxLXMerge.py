...
class ChooseColFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.listBox_left = wx.ListBox(
            self,
            -1,
            choices=self.columnsToChoose,
            name="",
            style=wx.LB_EXTENDED | wx.EXPAND,
            size=(200, 210),
        )
        self.button_toTheRight = wx.Button(self, -1, "->")
        self.button_toTheLeft = wx.Button(self, -1, "<-")
        self.label_right = wx.StaticText(
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.text_ctrl_inputFile2 = wx.TextCtrl(
            self, -1, "result.csv", style=wx.TE_PROCESS_ENTER, size=(300, 25)
        )
        ...
        self.checkBox_onlyFullOcc = wx.CheckBox(
            self, -1, "Output only fully occupied rows"
        )
        ...
        self.sizer_main_v.Add(
            self.button_start, 0, wx.ALL | wx.ALIGN_CENTER_HORIZONTAL, 5
        )
    ...
    def OnBrowse(self, evt):
        dlg = wx.FileDialog(
            self,
            "Select LipidXplorer result files for alignment.",
            style=wx.DD_DEFAULT_STYLE | wx.FD_MULTIPLE,
        )
    ...
    def OnBrowse2(self, evt):
        dlg = wx.FileDialog(
            self,
            "LipidX aligned spectra (from the MasterScan)",
            style=wx.DD_DEFAULT_STYLE | wx.FD_SAVE,
        )
    ...
    def OnStart(self, evt):
        ...
        self.colsToAlign = (
            self.text_ctrl_colsToAlign.GetValue().replace(" ", "").split(",")
        )
        ...
        if self.dictInputFiles == {}:
            dlg = wx.MessageDialog(
                self, "There are no input files", "Error", wx.ICON_ERROR
            )
            if dlg.ShowModal() == wx.ID_OK:
                dlg.Destroy()
                return None
                ...
                for key in list(row_sorted.keys()):
                    if key:
                        ...
                        if re.match(".*:.*", key):
                            ...
                            if (
                                not key in nonIntensityKeys
                                and not key in self.colsToAlign
                            ):
        ...
        self.win_chooseCols = ChooseColFrame(
        ...
        for entry in sorted(self.dictOutput.keys()):
            ...
            if self.onlyFullOccupation:
                ...
                for file in filenames:
                    for col in outputKeys[file]:
                        keys += 1
                        if (
                            col in self.dictOutput[entry][file]
                            or col in dictOutputSamples[entry][file]
                        ):
                            count += 1
            ...
            if cont:
                if countEntries % 10 == 0:
                    print(
                        "\n%d done - %d to go"
                        % (countEntries, lengthEntries - countEntries)
                    )
                else:
                    print(".", end=" ")
