...
@author: mirandaa
"""
from tools.reorderScans.reorderScansUI import MyFrame1
import wx
...
class MyFrame(MyFrame1):
        ...
        if not self.m_searchCtrl2.GetValue():
            ...
            if self.searchIncluding:
                self.selectionModel = [
                    ...
                    if self.m_searchCtrl2.GetValue() in item
                ]
            else:
                self.selectionModel = [
                    item
                    for item in self.sublist
                    if self.m_searchCtrl2.GetValue() not in item
                ]
    ...
    def toggleSearch(self, event):
        ...
        if self.searchIncluding:
            self.m_searchCtrl2.SetBackgroundColour(
    ...
    def clear_subandsearch(self, event):
        ...
        self.m_listBox1.Set(self.selectionModel)
        self.m_searchCtrl2.Clear()
        self.searchIncluding = (
            False  # false because next call will flip it  and update gui
        )
    ...
    def removeFromTargetList(self, event):
        self.destinationModel = [
            val
            for idx, val in enumerate(self.destinationModel)
            if idx not in self.m_listBox2.GetSelections()
        ]
        self.m_textCtrl_2.SetValue(str(len(self.destinationModel)))
        self.m_listBox2.Set(self.destinationModel)
