...
def main(folder , technial_replicates =True, rcf_a=5408000.0, rcf_b=0.5):
    ...
    if log.level == logging.DEBUG:log.addHandler(logging.StreamHandler()) # log to console
    if log.level == logging.DEBUG:logging.basicConfig(filename='peakStrainer.log',filemode='w')
...
#         scans = mzML2Scans(filename)
    ...
    preFiltered_filterlines = {k: preliminaryFilter(v) for k, v in list(filterLines.items())}
    #preFiltered_filterlines = {k: preliminaryReductionFilter(v) for k, v in filterLines.items()}
    if log.level == logging.DEBUG: filterlinePeaks_csv(preFiltered_filterlines,'preliminaryFilter.csv')    
    ...
    '''
    filterlines_withBins = {k: generateBins_resolutionPowerFunc(v, rcf_a, rcf_b ) for k, v in list(filterLines.items())}
    if log.level == logging.DEBUG: filterlinePeaks_csv(filterlines_withBins,'generateBins.csv')
...
#     for different function for ms and for msms
    ...
    filterlines_withBins = {k: alterBins_mergeOverlap(v) for k, v in list(filterlines_withBins.items())}
    if log.level == logging.DEBUG: filterlinePeaks_csv(filterlines_withBins,'mergeBins.csv')
    ...
    '''
    filterlines_inBins = {k: sortMassIn_FirstBin(v) for k, v in list(filterlines_withBins.items())}
    if log.level == logging.DEBUG: filterlinePeaks_csv(filterlines_inBins,'sortMassIn.csv')
    
    filterlines_binData = {k: aggregateBinData(v) for k, v in list(filterlines_inBins.items())}
    if log.level == logging.DEBUG: filterlineBins_csv(filterlines_binData,'aggregateBinData.csv')
    
    filterlines_filtered = {k: filterBins(v) for k, v in list(filterlines_binData.items())}
    if log.level == logging.DEBUG: filterlineBins_csv(filterlines_filtered,'filteredBins.csv')
    
    filtered_peaks = {k: bins2Peaks(v, filterlines_inBins[k]) for k, v in list(filterlines_filtered.items())}
    if log.level == logging.DEBUG: filterlinePeaks_csv(filtered_peaks,'mzXMLdata.csv')
...
def make_perFile(filtered_peaks):
    ...
    for filterline in filtered_peaks:
        ...
        for file in uniqFiles:
            ...
            fintens = [i for m,i,f in zip(masses, intens, files) if f == file]
...
#         out[:, 5] = asNumpyArray(data.Charges)
        ...
        return Labels(tuple(asNumpyArray(data.Masses)),
    ...
    for scanNum in range(start, end+1):
        ...
        if data.Length == 0:
            log.debug(f'scan {scanNum} {raw.GetScanEventForScanNumber(scanNum).ToString()} has no masses')
            continue
...
# (zmass, abunds, resols, baseline , noise, charge) = peakData
            ...
            peak_datas = (spectrum['m/z array'], spectrum['intensity array'], [None]*count,[None]*count,[None]*count,[None]*count)
...
# (zmass, abunds, resols, baseline , noise, charge) = peakData
            ...
            if len(spectrum['scanList']['scan']) > 1:
                log.error('scanList for single scan is more than 1 for: {}'.format(id))
...
def roundCollisionEnergy(scans, decimal_places = 6):
    log.info('roundCollisionEnergy in header to merge scans to %d decimal places', decimal_places)
    result = []
    for row in scans:
        ...
        if ' ms2 ' in filterLine:
            ...
            new_filterLine = m.groups()[0] +' '+ str(new_colisionEnergy) +'@'+ m.groups()[2]
...
def filterScanBy_retentionTime(scans, lowSeconds=0.5, highSeconds=float("inf")):
...
def mergePeaksOnFilterline(scans):
    ...
    filterLines = set([filterLine for scanNo, filterLine, peakData, retTime, file_name in scans])
...
def preliminaryFilter(peakData, decimal_places = 2, minCount = 2, minRepetitionRate = 0.0):
    log.debug('preliminaryFilter rounding to decimal_places %d, minCount = %d, minRepetitionRate %f',  decimal_places, minCount, minRepetitionRate)
    masses = peakData[0]
    ...
    minCount_bin = {bin:(count/3)+1 for bin,count in list(bins.items()) if (count/3)+1 >= minCount}
    log.debug('groups with minCount %d: %d', minCount,len(minCount_bin))
    minCount_bin_max = float(max(minCount_bin.values())) if len(minCount_bin) is not 0 else 0 
    minRepetitionRate_bin = {bin:count for bin,count in list(minCount_bin.items()) if (count/minCount_bin_max) > minRepetitionRate }
    log.debug('groups with minRepetitionRate %f: %d', minRepetitionRate,len(minRepetitionRate_bin))
    
    preFilterPass = [rounded in list(minRepetitionRate_bin.keys()) for rounded in roundeds]
    compressedPeakData = list(itertools.compress(list(zip(*peakData)), preFilterPass))
...
def preliminaryReductionFilter(peakData, reduction = 0.10, minPeaks = 10):
    ...
    log.debug('preliminaryReductionFilter will reduce peak count to max %d and min %d',  goal, minPeaks)
    ...
    while (peak_count > goal):
        if toggle: decimal_places += 1 
        else: minRepetitionRate +=0.1
        peakData = preliminaryFilter(peakData, decimal_places, minCount, minRepetitionRate)
        peak_count = len(peakData[0])
...
def generateBins_resolution(peakData, decimal_places = 4):
    ...
    bins_high = [mass + ((mass/resol)/2) for mass, resol in zip(masses,resols)] 
    bins_low =  [mass - ((mass/resol)/2) for mass, resol in zip(masses,resols)]
...
def peakWidth_at_hight(abunds, highth = 0.95):
    ...
    def widthAtHight(fwhm , normHight ):
        return 2 * math.sqrt(fwhm**2 * math.log(normHight) / (-4*math.log(2)))
    
    return [widthAtHight(abund, highth) for abund in abunds]
    
def generateBins_theoreticalResolution(peakData, decimal_places = 4):
    log.debug('generateBins_theoreticalResolution, decimal_places %d',  decimal_places)
    masses = peakData[0]
    abunds = peakData[1]
    resols = peakData[2]
    ...
    bins_high = [mass + ((mass/resol)/2) for mass, resol in zip(masses,theoResols)] 
    bins_low =  [mass - ((mass/resol)/2) for mass, resol in zip(masses,theoResols)]
     # rounding to make bins larger
    halfWidth= (10**-decimal_places)/2
    bins_low = [round(mass-halfWidth,decimal_places ) for mass in bins_low]
    bins_high = [round(mass+halfWidth,decimal_places ) for mass in bins_high]
...
def generateBins_resolutionPowerFunc(peakData, a = 1.0, minus_b = 1.0, decimal_places = 4):
    log.debug('generateBins_resolutionPowerFunc, mass +- res_func, res_func = a*(mass**-b), a= %f, b = %f',a, minus_b  )
    masses = peakData[0]
...
def generateBins_theoreticalIntensity(peakData, decimal_places = 4):
    log.debug('generateBins_theoreticalIntensity, decimal_places %d',  decimal_places)
    masses = peakData[0]
    abunds = peakData[1]
    resols = peakData[2]
    log.debug('generateBins_theoreticalResolution, peak count %d', len(masses))
    ...
    popt, pconv = curve_fit(func, select_masses[side_len:-side_len], select_resols[side_len:-side_len])
...
def alterBins_mergeOverlap(peakData):
    ...
    while True: #bubble check
        ...
        for idx, _ in enumerate(bins_high):
            if idx >= len(bins_high)-1 : continue 
            low = bins_low[idx]
            high = bins_high[idx]
            nextLow = bins_low[idx+1]
            nexthigh = bins_high[idx+1]
            
            if low== nextLow and high == nexthigh : continue # already updated
...
def sortMassIn_FirstBin(peakData):
    log.debug('sortMassIn_FirstBin, no sorting , just the first bins that matches')
    masses = peakData[0]
    abunds = peakData[1]
    resols = peakData[2]
    bins_low = peakData[-2]
    ...
    if (len(masses) * len(uniqBins) > 1000000):
        log.debug('this operation may take a long time, use sortMassIn_sortWindow instead')
    ...
    if(len(bins) != len(masses)):
        log.error('There was a problem setting masses in bins, please verify bins where created correctly')
        raise RuntimeError('There was a problem setting masses in bins, please verify bins where created correctly')
...
def sortPeaksByBinWidth(peakData):
    bin_width = [bin_high - bin_low for bin_low, bin_high in zip(peakData[-2], peakData[-1])]
...
def sortMassIn_NarrowestBin(peakData):
    ...
    log.debug('Mass count %d, available bins %d', len(masses), len(uniqBins))
    if (len(masses) * len(uniqBins) > 1000000):
        log.debug('this operation may take a long time, use sortMassIn_sortNarrowWindow instead')
...
def sortMassIn_sortWindow(peakData, window = 200):
    ...
    while tryAgain:
        ...
        for idx, mass in enumerate(masses):
            ...
            if idx % window == 0: log.debug('at %d of %d', idx, len(masses))
            for bin_low, bin_high in zip(bins_low[lowidx:highidx], bins_high[lowidx:highidx]):
                if mass >= bin_low and mass <= bin_high:
                    found = True
                    bins.append((bin_low,bin_high))
                    break 
            if not found:
                log.error('the mass at index %d did not find a bin, window size %d', idx, window)
                window = window * 2
    ...
    if(len(bins) != len(masses)):
        log.error('There was a problem setting masses in bins, please verify bins where created correctly, and window size')
...
def sortMassIn_sortNarrowWindow(peakData, window = 200):
    ...
    for idx, mass in enumerate(masses):
        ...
        #--
        bin_width = [bin_high - bin_low for bin_low, bin_high in zip(peakData_sorted[-2][lowidx:highidx], peakData_sorted[-1][lowidx:highidx])]
        sorted_peakdata_plus = list(zip(*sorted(zip(bin_width, peakData_sorted[0][lowidx:highidx],
...
def aggregateBinData(peakData, max_count = None):
    log.debug('aggregateBinData count %d, count, sumZmass, sumAbund, sum_Sig2Noise', len(peakData))
    masses = peakData[0]
    abunds = peakData[1]
    resols = peakData[2]
    bins_low = peakData[-3]
    ...
    for mass, abund, resol,baseline , noise, charge,file_name,  bin_low, bin_high, bin in zip(*peakData):
...
def getMaxCount(binData, disregardBottom = 0.10):
    ...
    log.debug('bins with average intensity lower than %f are not evaluated to establish min repetition rate', abundsCutoff)
    for bin in binData:
        count, sumZmass, sumAbund, sumNoise = binData[bin]
        if (sumAbund / count) < abundsCutoff: continue  # dont consider the
        if maxCount < count:
            maxCount = count
    
    log.debug('Max peak count is %d, for bins with average abundance above %f ', maxCount, abundsCutoff)
    return maxCount

def filterBins(binData, minRepetitionRate = 0.70, minSignal2Noise = 1.0, decimal_places = 4, disregardBottom= 0.10):
    decimal = (10 ** -decimal_places)
    log.debug('filterBins count %d ,minRepetitionRate %f, decimal places %d, %f, desrigarding bottom %f percent',len(binData),minRepetitionRate, decimal_places, decimal, disregardBottom)
    if disregardBottom >= 1 : raise RuntimeError('disregardBottom can not be more than 1')
...
#     for bin in binData:
    for idx, row in enumerate(sortedBintable):
        ...
        minSignal2Noise = 0 if sum_sig2Noise == 0 else minSignal2Noise # in casse there is no signal to noise
        if count >= (maxCount * minRepetitionRate) and (sum_sig2Noise/count) >= minSignal2Noise:
            binData_filtered[bin]=(count, sumZmass, sumAbund, sum_sig2Noise)

        if idx+1 >= len(sortedBintable): continue

        nextrow = sortedBintable[idx+1]  
        # test edge cases
        if abs(nextrow[0]-row[1])<decimal : # row highbin is adjecent nextrow lowbin
            count2 =row[2]+nextrow[2]
            sum_sig2Noise2 = row[5]+nextrow[5]
            if count2 >= (maxCount * minRepetitionRate) and (sum_sig2Noise2/count2) >= minSignal2Noise:
                binData_filtered[bin]=(count, sumZmass, sumAbund, sum_sig2Noise)
...
def bins2Peaks(binsData, peakData):
    log.debug('bins2Peaks get all the peaks with given bin, into simple list pair, mass and intentisty')
    ...
    filteredRowdata = [(row[0], row[1], row[6]) for row in zip(*peakData) if row[9] in bins_filtered]
...
def reorder4lipidxplorer(filtered_bins):
     ...
     filtered_bins1 = collections.OrderedDict(sorted(list(filtered_bins.items()), key=lambda t: t[0]))
     return filtered_bins1

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("folder", help="folder with technical replicates")
    parser.add_argument("--rcf_a", help="select the resolution curve finction value a", type=float, default=5408000.0 )
    parser.add_argument("--rcf_b", help="select the resolution curve finction value b", type=float, default=0.5)
    
    args = parser.parse_args()
    
    main(args.folder, technial_replicates =True, rcf_a=args.rcf_a, rcf_b=args.rcf_b)