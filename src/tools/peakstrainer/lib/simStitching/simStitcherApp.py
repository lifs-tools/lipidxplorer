...
@author: mirandaa
'''
from .simStitcherGUI import SimStitcherFrame
import wx
from lib.simStitching.simStitcher import simStitcher
import os
import traceback
...
#         simStitcher(filepath, doCompare)
        ...
        dlg = wx.FileDialog(self, message="Select your files",defaultDir=os.getcwd(),defaultFile="*.mzXML", wildcard=wildcard, style=wx.FD_OPEN|wx.FD_MULTIPLE)
...
#         print 'checkAdapt' + str(self.m_checkBox12.GetValue())
        ...
        if self.m_checkBox12.GetValue():
            dlg = wx.MessageDialog(self, 'This option is used to show SIM edges are not consistent\n please use with caution. ', 'Warning', wx.OK | wx.ICON_WARNING)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def get_simEdge(self):
        if self.m_radioBtn31.GetValue(): return 5 # its default
        try:
            return float(self.m_textCtrl4.GetValue())
        except ValueError:
            dlg = wx.MessageDialog(self, 'There was a problem with value {} \n it will not be used'.format(self.m_textCtrl4.GetValue()), 'Warning', wx.OK | wx.ICON_WARNING)
            dlg.ShowModal()
            dlg.Destroy()
...
#         cut1 = self.m_checkBox11.GetValue()
        daltons = self.get_simEdge() if self.m_checkBox11.GetValue() else 0.
        ...
        for onefile in self.fileNames:
            ...
            try:
                simStitcher(filepath, scaleToCenterMass, adaptByRegresion, daltons, csvPath1 = None, loglevel = loglevel)
                self.m_gauge1.SetValue(self.m_gauge1.GetValue() + gaugeStep)
                ststus = True
            except Exception as e:
                print(e)
                print(traceback.print_exc())
                dlg = wx.MessageDialog(self, 'There was a problem processing the file {} \n{} \n{}'.format(onefile, e,traceback.print_exc()), 'Warning', wx.OK | wx.ICON_WARNING)
                dlg.ShowModal()
                dlg.Destroy()
                self.m_gauge1.SetValue(self.m_gauge1.GetValue() - gaugeStep)
