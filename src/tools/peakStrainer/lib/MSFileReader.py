...
class ThermoRawfile(object):
    ...
    def Version(self):  # MSFileReader DLL version
        ...
        MajorVersion, MinorVersion, SubMinorVersion, BuildNumber = (
            c_long(),
            c_long(),
            c_long(),
            c_long(),
        )
        error = self.source.Version(
            byref(MajorVersion),
            byref(MinorVersion),
            byref(SubMinorVersion),
            byref(BuildNumber),
        )
        if error:
    ...
    def GetNumberOfControllersOfType(self, controllerType="MS"):
        ...
        if isinstance(controllerType, str):
    ...
    def GetSegmentAndEventForScanNum(
        self, scanNumber
    ...
    def GetMassRangeFromScanNum(self, scanNumber, massRangeIndex):
        ...
        error = self.source.GetMassRangeFromScanNum(
            c_long(scanNumber),
            c_long(massRangeIndex),
            byref(pdMassRangeLowValue),
            byref(pdMassRangeHighValue),
        )
        if error:
    ...
    def GetSourceFragmentValueFromScanNum(
        self, scanNumber, sourceFragmentIndex
    ):
    ...
    def GetNumberOfSourceFragmentationMassRangesFromScanNum(self, scanNumber):
        ...
        if error:
            raise IOError(
                "GetNumberOfSourceFragmentationMassRangesFromScanNum error :",
                error,
            )
        return result.value

    def GetSourceFragmentationMassRangeFromScanNum(
        self, scanNumber, sourceFragmentIndex
    ):
        ...
        error = self.source.GetSourceFragmentationMassRangeFromScanNum(
            c_long(scanNumber),
            c_long(sourceFragmentIndex),
        ...
        return (
            pdSourceFragmentRangeLowValue.value,
            pdSourceFragmentRangeHighValue.value,
        )
    ...
    def GetMassCalibrationValueFromScanNum(
        self, scanNumber, massCalibrationIndex
    ):
    ...
    def ScanNumFromRT(self, RT):
        ...
        if error:
            raise IOError(
                "scan {}, ScanNumFromRT error : {}".format(pnScanNumber, error)
            )
        else:
    ...
    def RTFromScanNum(self, scanNumber):
        ...
        if error:
            raise IOError(
                "scan {}, RTFromScanNum error : {}".format(
    ...
    def GetFilterForScanNum(self, scanNumber):
        ...
        if error:
            raise IOError(
                "scan {}, GetFilterForScanNum error : {}".format(
                    scanNumber, str(error)
                )
            )
        else:
    ...
    def GetMassListRangeFromScanNum(
        self,
        scanNumber,
        massRange="",
        filter="",
    ...
    def GetSegmentedMassListFromScanNum(
        self,
        scanNumber,
        filter="",
        intensityCutoffType=0,
        ...
        error = self.source.GetSegmentedMassListFromScanNum(
            byref(c_long(scanNumber)),
            filter,
            intensityCutoffType,
            intensityCutoffValue,
            maxNumberOfPeaks,
    ...
    def GetAverageMassList(
        ...
        peakFlags = comtypes.automation.VARIANT()
        pnArraySize = c_long()
        error = self.source.GetAverageMassList(
            byref(c_long(firstAvgScanNumber)),
    ...
    def GetAveragedMassSpectrum(self, listOfScanNumbers, centroidResult=False):
        ...
        peakFlags = comtypes.automation.VARIANT()
        pnArraySize = c_long()
        error = self.source.GetAveragedMassSpectrum(
            x,
            len(x),
            centroidResult,
            byref(peakList),
    ...
    def GetSummedMassSpectrum(self, listOfScanNumbers, centroidResult=False):
        ...
        peakFlags = comtypes.automation.VARIANT()
        pnArraySize = c_long()
        error = self.source.GetSummedMassSpectrum(
            x,
            len(x),
            centroidResult,
            byref(peakList),
    ...
    def GetLabelData(
        ...
        Flags = namedtuple(
            "Flags", "saturated fragmented merged exception reference modified"
        )
    ...
    def GetAveragedLabelData(self, listOfScanNumbers):
        ...
        peakFlags = comtypes.automation.VARIANT()
        pnArraySize = c_long()
        error = self.source.GetAveragedLabelData(
            x, len(x), byref(peakList), byref(peakFlags), byref(pnArraySize)
        )
        if error:
    ...
    def GetAllMSOrderData(self, scanNumber):  # same as GetLabelData ?
        ...
        return (
            Labels(*pvarLabels.value),
            Flags(*pvarFlags.value),
    ...
    def GetFullMSOrderPrecursorDataFromScanNum(self, scanNumber, MSOrder):
        ...
        return FullMSOrderPrecursorData(
            *pvarFullMSOrderPrecursorInfo.value[:8]
        )
    ...
    def GetMSOrderForScanNum(self, scanNumber):
        ...
        if error:
            raise IOError(
                "scan {} : GetMSOrderForScanNum error : {}".format(
                    scanNumber, error
                )
            )
    ...
    def GetPrecursorMassForScanNum(self, scanNumber, MSOrder):
        ...
        if error:
            raise IOError(
                "scan {} : GetPrecursorMassForScanNum error : {}".format(
                    scanNumber, error
                )
            )
    ...
    def GetPrecursorRangeForScanNum(self, scanNumber, MSOrder):
        ...
        pbIsValid = c_long()
        error = self.source.GetPrecursorRangeForScanNum(
            c_long(scanNumber),
            c_long(MSOrder),
            byref(pdFirstPrecursorMass),
            byref(pdLastPrecursorMass),
            byref(pbIsValid),
        )
        if error:
            raise IOError("GetPrecursorRangeForScanNum error :", error)
        return (
            pdFirstPrecursorMass.value,
            pdLastPrecursorMass.value,
            bool(pbIsValid.value),
        )

    # # # # # # # # # # # # # PRECURSOR END

    ############################################### XCALIBUR INTERFACE BEGIN
    def GetScanHeaderInfoForScanNum(
        self, scanNumber
    ...
    def GetTrailerExtraForScanNum(
        ...
        return OrderedDict(
            list(
                zip(
                    [str(x[:-1]) for x in labels.value],
                    list(map(_to_float, values.value)),
                )
            )
        )
    ...
    def GetInstMethod(
        self, instMethodItem=0
    ):  # "View/Report/Instrument Method"
        """Returns the channel label, if available, at the specified index for the current controller. This
        field is only relevant to channel devices such as UV detectors, A/D cards, and Analog inputs.
    ...
    def GetStatusLogForScanNum(self, scanNumber):  # "View/Report/Status Log"
        ...
        pvarValues = comtypes.automation.VARIANT()
        pnArraySize = c_long()
        error = self.source.GetStatusLogForScanNum(
            c_long(scanNumber),
            byref(pdStatusLogRT),
            pvarLabels,
            pvarValues,
        ...
        return pdStatusLogRT.value, list(
            zip(
                [label.rstrip(":") for label in pvarLabels.value],
                pvarValues.value,
            )
        )
    ...
    def GetStatusLogForPos(self, position=0):
        ...
        pvarValues = comtypes.automation.VARIANT()
        pnArraySize = c_long()
        error = self.source.GetStatusLogForPos(
            c_long(position),
            byref(pvarRT),
            byref(pvarValues),
            byref(pnArraySize),
    ...
    def GetErrorLogItem(self, itemNumber=1):  # "View/Report/Error Log"
        ...
        pdRT = (
            c_double()
        )  # A valid pointer to a variable of type double to receive the retention time when the error occurred. This variable must exist.
        pbstrErrorMessage = comtypes.automation.BSTR()
    ...
    def GetChroByCompoundName(
        self,
        compoundNamesList=None,
        startTime=0.0,
        endTime=0.0,
        massRanges1="",
        massRanges2="",
        ...
        pvarPeakFlags = comtypes.automation.VARIANT()
        pnArraySize = c_long()
        error = self.source.GetChroByCompoundName(
            c_long(chroType1),
            c_long(chroOperator),
            c_long(chroType2),
        ...
        return (
            pvarChroDataArray.value,
            pvarChroParamsArray.value,
            pvarPeakFlagsArray.value,
        )
    ...
    def _GetStatusLogLabelsForScanNum(
        self,
    ):  # REDUNDANT with GetStatusLogForScanNum
        pass

    def _GetStatusLogLabelsForRT(
        self,
    ):  # REDUNDANT with GetStatusLogForScanNum
        pass

    def _GetStatusLogValueForScanNum(
        self,
    ):  # REDUNDANT with GetStatusLogForScanNum
        pass
    ...
    def _GetTrailerExtraForRT(
        self,
    ):  # REDUNDANT with GetTrailerExtraForScanNum
        pass

    def _GetTrailerExtraLabelsForScanNum(
        self,
    ):  # REDUNDANT with GetTrailerExtraForScanNum
        pass

    def _GetTrailerExtraLabelsForRT(
        self,
    ):  # REDUNDANT with GetTrailerExtraForScanNum
        pass

    def _GetTrailerExtraValueForScanNum(
        self,
    ):  # REDUNDANT with GetTrailerExtraForScanNum
        pass

    def _GetTrailerExtraValueForRT(
        self,
    ):  # REDUNDANT with GetTrailerExtraForScanNum
        pass
    ...
    def _GetPrevMassListRangeFromScanNum(
        self,
    ):  # REDUNDANT with GetMassListRangeFromScanNum
        pass

    def _GetMassListRangeFromRT(
        self,
    ):  # REDUNDANT with GetMassListRangeFromScanNum
        pass

    def _GetNextMassListRangeFromScanNum(
        self,
    ):  # REDUNDANT with GetMassListRangeFromScanNum
        pass
    ...
    def _GetNextMassListFromScanNum(
        self,
    ):  # REDUNDANT with GetMassListFromScanNum
        pass

    def _GetPrevMassListFromScanNum(
        self,
    ):  # REDUNDANT with GetMassListFromScanNum
        pass
    ...
    def _GetSegmentedMassListFromRT(
        self,
...
if __name__ == "__main__":
    ...
    print(
        ...
        rawfile.SetMassTolerance(
            userDefined=True, massTolerance=555.0, units=2
        ),
    )
    print("GetMassTolerance()", rawfile.GetMassTolerance())
    print(
        ...
        rawfile.SetMassTolerance(
            userDefined=False, massTolerance=500.0, units=0
        ),
    )
    ...
    print(
        ...
        rawfile.GetScanHeaderInfoForScanNum(scanNumber),
    )  # "View/Scan header", upper part
    print(
        "GetTrailerExtraForScanNum",
        rawfile.GetTrailerExtraForScanNum(scanNumber),
    )  # "View/Scan header", lower part
    print("GetNumTuneData", rawfile.GetNumTuneData())
    print(
        "GetTuneData(0)", rawfile.GetTuneData(0)
    ...
    print(
        "rawfile.ExtractInstMethodFromRaw",
    ...
    for i in range(rawfile.GetNumErrorLog()):
        print(
            "GetErrorLogItem", i, rawfile.GetErrorLogItem(i)
    ...
    print(
        "GetAverageMassList",
        rawfile.GetAverageMassList(scanNumber, scanNumber + 10),
    )
    print(
    ...
    print(
        "GetPrecursorInfoFromScanNum(scanNumber,1)",
    ...
    with open("test.tsv", "wt") as f:
        ...
        for i in range(
            rawfile.FirstSpectrumNumber, rawfile.LastSpectrumNumber + 1
        ):
            print(
                "\t".join(
                    map(
