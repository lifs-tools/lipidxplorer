...
def add_Sample(sc=None, specFile=None, specDir=None, options={}, **kwargs):
    ...
    if options["timerange"]:
        ...
        reportout(
            "Limiting timerange from %f to %f (sec), %f to %f (min)"
            % (options["timerange"][0], options["timerange"][1], t1, t2)
        )
    ...
    if len(ms1Scans) > 0:
        ...
        for ms1scan_entry in ms1Scans:
            min_da_delta_all_scans = min(
                min_da_delta_all_scans, ms1scan_entry["min_da_delta"]
            )
        ...
        if (
            ...
            df["sample_or_retention_time"] = df[
                "sample_or_retention_time"
            ].astype("category")
    ...
    if len(ms2Scans) > 0:
        ...
        if kwargs.get("peaks_df_list", None) is not None:
            peaks = (
                (
                    specName,
                    -1,
            ...
            df_ms2["sample_or_retention_time"] = df_ms2[
                "sample_or_retention_time"
            ].astype("category")
