...
def make_SMH1(dictQuery, count):
    ...
    SMH_df["abundance_variation_study_variable[1]"] = dataDict.get(
        "ERRppm", None
    )

    as_text = (
        SMH_df.fillna("null")
        .to_csv(None, sep="\t", index=False)
        .replace("\r\n", "\n")
    )
    return as_text
...
def make_MTD2(dictQuery, count):
    ...
    MTD_dict[
        f"ms_run[{count}]-format"
    ] = "[,, LipidXplorer mzTab output, 1.3.0]"
    MTD_dict[
    ...
    MTD_dict[
        f"ms_run[{count}]-scan_polarity[{count}]"
    ] = f"[MS, MS:1000129, {mode}, ]"
...
def make_MTD3(dictQuery, count):
    ...
    for idx, k in enumerate(dictQuery.sc[0].dictScans.keys()):
        ...
        MTD_dict[
            f"study_variable[{count+idx}]-assay_refs"
        ] = f"assay[{count+idx}]"
...
def as_mztab(result):
    ...
    for idx, k in enumerate(result.dictQuery, start=1):
        txt += (
            make_MTD2(result.dictQuery[k], idx)
            if result.dictQuery[k].sc
            else ""
        )
        txt += (
            make_MTD3(result.dictQuery[k], idx)
            if result.dictQuery[k].sc
            else ""
        )
    txt += make_MTD4()
    for idx, k in enumerate(result.dictQuery, start=1):
        txt += (
            make_SMH1(result.dictQuery[k], idx)
            if result.dictQuery[k].sc
            else ""
        )
